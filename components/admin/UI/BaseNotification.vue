<template>
  <div class="fixed z-40 top-0 px-50 py-2 w-2/3 text-lg text-center">
    <div
      class="px-6 py-4 mx-2 my-4 rounded-md text-lg flex items-center mx-auto w-3/4 xl:w-2/4"
      :class="{ 'bg-green-200': success, 'bg-red-200': error }"
    >
      <span v-if="success">
        <svg
          viewBox="0 0 24 24"
      ...
      <span :class="{ 'text-green-800': success, 'text-red-200': error }">
        {{ text }}
      </span>
    </div>
  </div>
